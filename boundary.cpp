    ...
    while (getline(surface_command_file, buffer)) {
        if (is_substr(buffer, "Random hummocks")) {
            ...
            if (sscanf(buffer.c_str(), "%*[^=]=%lf;", &rand_humm_number) == 1 && !is_extra_symbol_after_semicolon(buffer)) {
                print_message_in_beam("Command to define number of random hummocks has been read! ");
                if (controller->surface_set_random_hills(rand_humm_number) == 0) {
                    print_message_in_beam("The Controller did not accept these values ");
                ...
                print_message_in_beam("ERROR! Wrong command to set number of random hummocks. You need to write 1 positive integer number as described in config sample and don`t write something after semicolon! ");
                surface_command_file.close();
                return 0;
            }
        }
            ...
            if (sscanf(buffer.c_str(), "%*[^=]=%lf;", &temp) == 1 && is_extra_symbol_after_semicolon(buffer) == 0) {
                print_message_in_beam("Command to define number of random stones has been read! ");
                ...
                print_message_in_beam("ERROR! Wrong command to set number of random stones. You need to write 1 number as described in config sample and don`t write something after semicolon! ");
                surface_command_file.close();
                return 0;
            }
        }
            ...
            double temp;
            if (sscanf(buffer.c_str(), "%*[^=]=%lf;", &temp) == 1 && is_extra_symbol_after_semicolon(buffer) == 0) {
                print_message_in_beam("Command to define number of random logs has been read! ");
                if (controller->surface_set_random_beams(temp) == 0) {
                    print_message_in_beam("The Controller did not accept these values ");
                    surface_command_file.close();
                    return 0;
                }
                ...
                print_message_in_beam("ERROR! Wrong command to set number of random logs. You need to write 1 number as described in config sample and don`t write something after semicolon! ");
                surface_command_file.close();
                return 0;
            }
        }
            ...
            if (sscanf(buffer.c_str(), "%*[^=]=%lf,%*[^=]=%lf,%*[^=]=%lf;", &temp1, &temp2, &temp3) == 3 && is_extra_symbol_after_semicolon(buffer) == 0) {
                print_message_in_beam("Command to create surface has been read! ");
                ...
                print_message_in_beam("ERROR! Wrong command to create surface. You need to write 3 numbers as described in config sample and don`t write something after semicolon! ");
                surface_command_file.close();
                return 0;
            }
        }
            ...
            if (sscanf(buffer.c_str(), "%*[^(](%lf, %lf)", &temp1, &temp2) == 2 && is_extra_symbol_after_semicolon(buffer) == 0) {
                print_message_in_beam("Command to define cursor location has been read!");
                ...
                print_message_in_beam("ERROR! Wrong command to set cursor location. You need to write 2 numbers in parentheses separated by commas as described in config sample and don`t write something after semicolon! ");
                surface_command_file.close();
                return 0;
            }
        }
            ...
            if (sscanf(buffer.c_str(), "%*[^(](%lf, %lf, %lf, %lf);", &temp1, &temp2, &temp3, &temp4) == 4 && is_extra_symbol_after_semicolon(buffer) == 0) {
                print_message_in_beam("Command to create hummock has been read! ");
                if (controller->surface_create_hill(temp1, temp2, temp3, temp4) == 0) {
                    print_message_in_beam("The Controller did not accept these values ");
                    surface_command_file.close();
                    return 0;
                }
                ...
                print_message_in_beam("ERROR! Wrong command to create hummock. You need to write 4 numbers in parentheses separated by commas as described in config sample and don`t write something after semicolon! ");
                surface_command_file.close();
                return 0;
            }
        }
            ...
            double temp;
            if (sscanf(buffer.c_str(), "%*[^(](%lf);", &temp) == 1 && is_extra_symbol_after_semicolon(buffer) == 0) {
                print_message_in_beam("Command to create stone has been read! ");
                ...
                print_message_in_beam("ERROR! Wrong command to create stone. You need to write 1 number in parentheses as described in config sample and don`t write something after semicolon! ");
                surface_command_file.close();
                return 0;
            }
        }
            ...
            if (sscanf(buffer.c_str(), "%*[^(](%lf, %lf, %lf, %lf, %lf);", &temp1, &temp2, &temp3, &temp4, &temp5) == 5 && is_extra_symbol_after_semicolon(buffer) == 0) {
                print_message_in_beam("Command to create log has been read! ");
                if (controller->surface_create_beam(temp1, temp2, temp3, temp4, temp5) == 0) {
                    print_message_in_beam("The Controller did not accept these values ");
                    surface_command_file.close();
                    return 0;
                }
                ...
                print_message_in_beam("ERROR! Wrong command to create log. You need to write 5 numbers in parentheses separated by commas as described in config sample and don`t write something after semicolon! ");
                surface_command_file.close();
                return 0;
            }
        }
            ...
            if (is_extra_symbol_after_semicolon(buffer) == 0) {
                print_message_in_beam("Command to count surface has been read! ");
                ...
                print_message_in_beam("ERROR! Wrong command to count surface! Try delete extra symbols after semicolon! ");
                surface_command_file.close();
                return 0;
            }
        }
        else {
            print_message_in_beam("ERROR! UNKNOWN COMMAND! ");
            surface_command_file.close();
            return 0;
        }
    }
...
void Boundary::print_message_in_beam(const string& message)
{
    if (is_date) {
    ...
    while (getline(rover_command_file, buffer))
    {
        if (is_substr(buffer, "Create Rover")) {
            ...
            if (sscanf(buffer.c_str(), "%*[^(](%d, %d, %lf, %lf, %lf, %lf);", &temp1, &temp2, &temp5, &temp6, &temp7, &temp8) != 0 && is_extra_symbol_after_semicolon(buffer) == 0) {
                print_message_in_beam("Command to create rover has been read! ");
                controller->rover_create(temp1, temp2, temp5, temp6, temp7, temp8);
                controller->route();
                is_rover = true;
            }
            else {
                print_message_in_beam("ERROR! Wrong command to create rover. You need to write 10 numbers as described in config sample and don`t write something after semicolon! ");
                rover_command_file.close();
                return 0;
            }
        }
        else if (is_rover && is_substr(buffer, "turn 180;")) {
            print_message_in_beam("Command to turn rover has been read! ");
            controller->rover_turn_180();
        }
        else if (is_rover && is_substr(buffer, "turn 90 clockwise;")) {
            print_message_in_beam("Comand to turn rover has been read! ");
            ...
            print_message_in_beam("Comand to move the rover has been read! ");
            ...
            print_message_in_beam("ERROR! Wrong command. You can constructe only one rover! ");
            rover_command_file.close();
            return 0;
        }
        else {
